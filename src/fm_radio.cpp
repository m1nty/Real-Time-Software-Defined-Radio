...
void mono_stero_thread(int &mode, std::queue<std::vector<float>> &sync_queue, std::mutex &radio_mutex, std::condition_variable &cvar) 
{
	...
	while (true) 
	{
		...
		if(mode == 1)
		{
			convolveWithDecimMode1(audio_block, sync_queue.front(), mono_coeff, audio_inital, audio_decim, audio_up);
			mult = 24;
		}
		//if in mode 0 
		else
		{
			convolveWithDecim(audio_block, sync_queue.front(), mono_coeff, audio_inital, audio_decim);
		}
		...
		for(unsigned int l=0; l<audio_block.size(); l++)
		{
			...
			{
				audio_data[l] = static_cast<short int>(audio_block[l] *16384*mult);
			}
		}
